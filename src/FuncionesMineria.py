...
def lm(varObjCont, datos, var_cont, var_categ, var_interac=[]):
    ...
    # 1) Convertir categóricas restantes (por si alguna se coló) a dummies
    datos = pd.get_dummies(datos, drop_first=True)

    # 2) Forzar todo a numérico y luego a float (esto elimina dtype object sí o sí)
    datos = datos.apply(pd.to_numeric, errors="coerce").astype(float)
    
    # 3) Asegurar que y también sea numérica
    y_ser = pd.to_numeric(pd.Series(varObjCont, index=datos.index, name="y"), errors="coerce")
    tmp = pd.concat([y_ser, datos], axis=1).dropna()


    # 4) Alinear X e y y quitar NaNs en conjunto (MUY importante)
    y_ser = pd.Series(varObjCont, index=datos.index, name="y")
    tmp = pd.concat([y_ser, datos], axis=1).dropna()
    varObjCont = tmp["y"]
    datos = tmp.drop(columns=["y"])
